...
def main():
    ...
    print("Created method references for SmartMovingFeatures")

    m_name_idx = rewriter.get_utf8_idx(b"afterOnUpdate")
    m_desc_idx = rewriter.get_utf8_idx(b"()V")

    if not m_name_idx or not m_desc_idx:
        print("ERROR: afterOnUpdate method not found")
        return
    ...
    if not target_method:
        print("ERROR: afterOnUpdate method body not found")
        return

    print("Found afterOnUpdate method")
    ...
    for attr_idx, (name_idx, attr_body) in enumerate(target_method['attrs']):
        ...
        if name == b"Code":
            ...
            new_attr_body += struct.pack('>H', et_len)
            new_attr_body += attr_body[et_pos + 2:]
