...
def main():
    ...
    fh_e_fieldref           = rewriter.ef(b"fh", b"e", b"Lps;")
    fh_d_fieldref           = rewriter.ef(b"fh", b"d", b"Lps;")
    mp_e_fieldref           = rewriter.ef(b"net/minecraft/move/ModelPlayer", b"e", b"Lnet/minecraft/move/ModelRotationRenderer;")
    mp_d_fieldref           = rewriter.ef(b"net/minecraft/move/ModelPlayer", b"d", b"Lnet/minecraft/move/ModelRotationRenderer;")
    mrr_class               = rewriter.ec(b"net/minecraft/move/ModelRotationRenderer")
    ...
    print(f"fh.e fieldref: #{fh_e_fieldref}")
    print(f"fh.d fieldref: #{fh_d_fieldref}")
    print(f"ModelPlayer.e fieldref: #{mp_e_fieldref}")
    print(f"ModelPlayer.d fieldref: #{mp_d_fieldref}")
    print(f"ModelRotationRenderer class: #{mrr_class}")
    ...
    for attr_idx, (name_idx, attr_body) in enumerate(target_method['attrs']):
        ...
        if name == b"Code":
            ...
            # Construct injected bytecode sequence (25 + 13 = 38 bytes)
            # 1. Arm swapping (25 bytes)
            #    aload_0 (0x2A)
            #    getfield modelBipedMain (0xB4, high, low)
            #    dup (0x59)
            #    dup (0x59)
            #    getfield fh.e (0xB4, high, low)
            #    checkcast ModelRotationRenderer (0xC0, high, low)
            #    putfield ModelPlayer.e (0xB5, high, low)
            #    dup (0x59)
            #    getfield fh.d (0xB4, high, low)
            #    checkcast ModelRotationRenderer (0xC0, high, low)
            #    putfield ModelPlayer.d (0xB5, high, low)
            ...
            injection = bytes([
                ...
                0x59,
                0x59,
                0xB4, (fh_e_fieldref >> 8) & 0xFF, fh_e_fieldref & 0xFF,
                0xC0, (mrr_class >> 8) & 0xFF, mrr_class & 0xFF,
                0xB5, (mp_e_fieldref >> 8) & 0xFF, mp_e_fieldref & 0xFF,
                0x59,
                0xB4, (fh_d_fieldref >> 8) & 0xFF, fh_d_fieldref & 0xFF,
                0xC0, (mrr_class >> 8) & 0xFF, mrr_class & 0xFF,
                0xB5, (mp_d_fieldref >> 8) & 0xFF, mp_d_fieldref & 0xFF,
            ...
            # Attributes of Code (shift offsets by +38)
            p_sub = et_pos + 2
            ...
            for _ in range(sub_ac):
                ...
                if sa_name == b"LineNumberTable":
                    ...
                    for _ in range(lnt_len):
                        ...
                        struct.pack_into('>H', sa_body, p_lnt, start_pc + 38)
                        p_lnt += 4
                    print(f"Shifted LineNumberTable ({lnt_len} entries) by +38")
                    ...
                    for _ in range(lvt_len):
                        ...
                        if start_pc == 0:
                            struct.pack_into('>H', sa_body, p_lvt + 2, length + 38)
                        else:
                            struct.pack_into('>H', sa_body, p_lvt, start_pc + 38)
                        p_lvt += 10
                    print(f"Shifted LocalVariableTable ({lvt_len} entries) by +38")
                elif sa_name == b"StackMapTable":
                    shift_smt(sa_body, 0, 38)
                    
                new_sub_attrs += struct.pack('>HI', sa_name_idx, len(sa_body)) + sa_body
                p_sub += 6 + sa_len
                
            # Reassemble Code attribute body
            # Original max_stack is 10. Our peak stack is 4 (for arm swap), so 10 is plenty.
